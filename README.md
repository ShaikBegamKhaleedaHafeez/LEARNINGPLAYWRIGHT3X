# Learning Playwright 3X — JavaScript Foundations

A structured journey through JavaScript fundamentals, building toward Playwright test automation. Each chapter focuses on core JS concepts with hands-on coding examples and IQ (Intelligent Question) notes.

## 📁 Chapters

| Chapter | Topic | Files |
|---------|-------|-------|
| **00 — GenAI** | Introduction to Generative AI | — |
| **01 — Basics** | Hello World & JS setup | `HelloWorld.js` |
| **02 — JS Basics** | JavaScript fundamentals | — |
| **03 — Identifiers** | Variable naming rules, comments | `03_Identifier_Rules.js`, `04_Identifier_Rules_Part2.js`, `05_Comments.js`, `06_Identifier_IQ.js` |
| **04 — Literals** | Values, null vs undefined | `07_Literal.js`, `08_null_undefined.js`, `09_Null_IQ.js` |
| **05 — Operators** | Arithmetic, comparison, logical, confusing operators | `13_DataTypes.js`, `15_Comparison_operator.js`, `16_Logical_Operators.js`, `17_Confusing_Operator.js` |
| **IQ_Notes** | Concept breakdowns (tables, diagrams, takeaways) | `Source_Code_ByteCODE_Binary_IQ.md`, `__prompt.md` |

## 🎯 Goal

Build a solid JS foundation — identifiers, literals, data types, operators — before diving into Playwright for browser automation.

## 🛠️ How to Run

```bash
node <filename>.js
```

Example:
```bash
node chapter_01_Basics/HelloWorld.js
```
